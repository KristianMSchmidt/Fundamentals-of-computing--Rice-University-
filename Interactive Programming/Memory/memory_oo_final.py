...
## MIx af min egen implementation og lærerimplementation
#################################################
...
def new_game():
    ...
    #my_tiles=[]
    #for i in [1,2]:
    #    for j in range(DISTINCT_TILES):
    #        n=(i-1)*DISTINCT_TILES+j
    #        my_tiles.append(Tile(tile_numbers[n], False, [TILE_WIDTH * j, TILE_HEIGHT*i]))

    my_tiles = [Tile(tile_numbers[i], False, [TILE_WIDTH * (i % DISTINCT_TILES) , (1+i//DISTINCT_TILES)*TILE_HEIGHT]) for i in range(2 * DISTINCT_TILES)]
