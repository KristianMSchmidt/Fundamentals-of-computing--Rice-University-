...
def draw(canvas):
    ...
    if time%SPEED==0:
        ...
        #update worm and head-history
        OLD_HEAD_HISTORY=list(HEAD_HISTORY)
        NEW_WORM=[]
        for i in range(WORM_LENGTH):
            NEW_WORM.append(OLD_HEAD_HISTORY.pop())

        HEAD_HISTORY=HEAD_HISTORY[-WORM_LENGTH:]
    ...
    if GAME_IS_ON:
       ...
       # if len(lst)+len(OLD_WORM)>GRID_SIZE**2 + 100:
        #    new_game()
