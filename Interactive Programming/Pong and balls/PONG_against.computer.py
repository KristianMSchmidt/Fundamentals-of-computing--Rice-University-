...
PADLET_SPEED_FACTOR=0.50 #0.62 også okay
DEFAULT=True
VERTICAL=False
GRAVITY =False
COMPUTER_PLAYS_LEFT=False
COMPUTER_PLAYS_RIGHT=True
...
def draw_game_is_on(canvas):
    ...
    global target_is_calculated #real_y_hit

    x=min([PADLET_SPEED_FACTOR*(abs(ball_vel[0])+abs(ball_vel[1])),150])

    global real_y_hit

    if COMPUTER_PLAYS_LEFT:


        #if ball_vel[0]==0:
         #   real_y_hit=HEIGHT/2
          #  print "ball velocity is zero. target",real_y_hit
            #target_is_calculated=False


        if ball_vel[0]<0 and number_of_saves==0:
            target_is_calculated=False
            real_y_hit=ball_pos[1]
            print "first, ball going left. target: ", real_y_hit

        elif ball_vel[0]>0:
            real_y_hit=HEIGHT/2
            print "ball going right. target", real_y_hit
            target_is_calculated=False

        elif ball_vel[0]<0 and ball_pos[0]>(WIDTH-PAD_WIDTH-4*BALL_RADIUS) and not target_is_calculated: #and (ball_vel[0]<0): #and not target_is_calculated):

            time_to_hit =(ball_pos[0]-BALL_RADIUS-PAD_WIDTH)/(-ball_vel[0])
            y_hit=int(time_to_hit*ball_vel[1])

            if (abs(y_hit)//HEIGHT)%2==0:
                real_y_hit=abs(y_hit)%HEIGHT
            else:
                real_y_hit=HEIGHT-(abs(y_hit)%HEIGHT)

            print "special case", real_y_hit

            target_is_calculated=True
        ...
        #if not time_is_calculated and not (ball_pos[0]<PAD_WIDTH+BALL_RADIUS+50 and ball_vel[0]>0):




    if COMPUTER_PLAYS_RIGHT:
