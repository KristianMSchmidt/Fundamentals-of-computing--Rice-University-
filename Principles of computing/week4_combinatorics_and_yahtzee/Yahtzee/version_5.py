...
"""
Automatic Yahtzee player.

This version is identical to "best version" except that I have tried
to make strategy function for the first hold decision.

I think it works, but it is extremely slow...

The strategy function is not yet implementet in "play round", but it shold be very
easy to do this.
"""
...
class YahtzeePlayer:
    ...
    def __str__(self):
        """
        Make string representation of player's score notebook
        """
    ...
    def get_game_state(self):
        """
        Returns number of game objectives met: 0, 1, 2, 3,...12, or 13
        """
        game_state = 0
    ...
    def choose_objective(self):
        ...
        #This rule hasn't proved to mean much in empirical tests, so can be omitted
    ...
    def strategy_after_second_roll(self, hand):
        """
        Compute the hold that maximizes the (absolute) expected value after the dice
        has been rolled for the second time.

        Returns the dice to hold for next round and the expected value of the
        resulting throw of the discarted dice
        """
        holds = list(gen_all_holds(hand))
        exp_vals = [self.expected_value(hold) for hold in holds]
        max_exp_val = max(exp_vals)
        pos_of_best_hold = exp_vals.index(max_exp_val)
        return (holds[pos_of_best_hold], max_exp_val)

    def strategy_after_first_roll(self):
        holds = list(gen_all_holds(self.hand))
        print "Starting with hand:", self.hand
        print "There are", len(holds), "possible holds."
        exp_vals_of_holds = []
        for hold in holds:
            exp_val_of_hold = 0
            num_free_dice = 5 - len(hold)
            all_outcomes = gen_all_sequences(range(1,7),num_free_dice)
            num_outcomes = len(all_outcomes)
            for outcome in all_outcomes:
                exp_val_hold_and_outcome = self.strategy_after_second_roll(hold + outcome)[1]
                exp_val_of_hold += (1.0/num_outcomes) * exp_val_hold_and_outcome
            exp_vals_of_holds.append(exp_val_of_hold)
            print "Holding", hold, "has an expexted value of", exp_val_of_hold
            print ""
            print "With old calculation, this same hold has an expected value of", self.strategy_after_second_roll(hold)
        max_exp_val = max(exp_vals_of_holds)
        pos_of_best_hold = exp_vals_of_holds.index(max_exp_val)
        return (holds[pos_of_best_hold], max_exp_val)
    ...
    def play_a_round(self):
        ...
        while round_not_over:
            ...
            hold = self.strategy_after_second_roll(self.hand)[0]
...
def test():
    ...
    player.strategy_after_first_roll()
test()
...
def get_average(number):
    """
    Plays full game any number of times.
    ...
    ##results wite 1's 5/6 adhoc rule:
    ##100 games 187,7poinys (with chance rule: if rel_scoe[11]<0.97, rel_scpre[11]=0.0
    ##50 games 175  (chance rule more simple rel[11]=0.0)
    ## 50 games__ no special chance rule: average=198 points

    #whithout any rules: 200.9 points in 50 games
    #with only 1-5-6 rule: score more than 200!
