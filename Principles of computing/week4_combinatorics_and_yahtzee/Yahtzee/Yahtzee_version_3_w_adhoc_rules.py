...
Last edited 12/11 by Kristian Moeller Schmidt, Copenhagen.

Weakness in algorithm: When deciding what to hold after first roll of the dice in each round, the
machine player makes its decision based only on the expected value after next roll. A more clever player
would base his hold decision on the fact that there are two possible rolls left. The current algorithm
may sometimes lead to overly conservative hold decisions after first throw.
"""
...
OPTIMAL_SCORES = (5, 10, 15, 20, 25, 30, 30, 30, 25, 30, 40, 30, 50)
...
class YahtzeePlayer:
    ...
    def choose_objective(self):
        ...
        Chooses according to the following rule:
            ...
            5. Finally there are some (wise??) adhoc rules, to avoid some obviously bad decisions.
               My ad hoc rules are unscientific and deserves more consideration.
        ...
        #first choices, based on relative score
        chosen_objective = relative_scores.index(max_rel_score)
        chosen_score = scores[chosen_objective]
        print "first choice", chosen_objective

        #check for better choices, based on absolute score (but don't change to 'chance')
        for indx in range(13):
            if relative_scores[indx] == max_rel_score:
                if scores[indx] > chosen_score and indx != 11:
                    chosen_score = scores[indx]
                    chosen_objective = indx
        print chosen_objective

        #adhoc rule 1: Choose smallest of objectives 0 to 5, when bad relative result (only one good die).
        if chosen_objective <= 5 and max_rel_score <= 0.20:
            for indx in range(6):
                if relative_scores[5-indx] == max_rel_score:
                    chosen_score = scores[5-indx]
                    chosen_objective = 5-indx
        print "after ad hoc rule 1:", chosen_objective

        #adhoc rule 2: It's better to chose 0 points on 1's than to cash in only 5 or 6 points on 5's or 6's
        if chosen_objective in [4, 5] and max_rel_score <= 0.20:
            if scores[0] != None:
                chosen_score = scores[0]
                chosen_objective = 0
        print "after ad hoc rule 2:", chosen_objective

        #ad hoc rule 3: Hesitate to play 'chance' if the relative score is not good. Look for other options.
        if chosen_objective == 11 and max_rel_score < 0.70:
            index = 11
            if scores[0] != None:
                index = 0
            if scores[1] > 0:
            ...
            chosen_objective = index
        print "after ad hoc rule 3:", chosen_objective

        #update player's notebook
        self.score_notes[chosen_objective] = chosen_score

        #return values for printing purposes
    ...
    def play_a_round(self):
        ...
        Prints out relavent information to the screen.
        """
